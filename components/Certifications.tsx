...
export default function Certifications() {
  return (
    <section id="certs" style={{ background:'#141414', padding:'clamp(60px,10vw,96px) 0' }}>
      <div style={{ maxWidth:1360, margin:'0 auto', padding:'0 clamp(16px,4vw,48px)' }}>
        ...
        <div className="certs-intro-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'clamp(32px,5vw,68px)', alignItems:'center', marginBottom:'clamp(36px,6vw,56px)' }}>
          <div>
            ...
            <h2 style={{ fontFamily:'var(--font-cormorant),serif', fontStyle:'italic', fontSize:'clamp(28px,6vw,50px)', fontWeight:700, color:'#fff', lineHeight:1.08, margin:'0 0 4px' }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:20, padding:'clamp(22px,4vw,34px)' }}>
            <div style={{ fontFamily:'var(--font-oswald),sans-serif', fontSize:12.5, fontWeight:600, letterSpacing:'2px', textTransform:'uppercase', color:'#F97316', marginBottom:18 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              {buyerPoints.map(p => (
                <div key={p} style={{ display:'flex', alignItems:'flex-start', gap:11 }}>
                  ...
                  <p style={{ fontFamily:'var(--font-inter),sans-serif', fontSize:'clamp(13px,3vw,14px)', color:'rgba(255,255,255,0.48)', lineHeight:1.65, margin:0 }}>{p}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="cert-grid-responsive" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'clamp(12px,2.5vw,20px)' }}>
          {certs.map(cert => (
            <div key={cert.name}
              ...
              <div style={{ width:'clamp(68px,12vw,82px)', height:'clamp(68px,12vw,82px)', borderRadius:'50%', background:'rgba(255,255,255,0.95)', border:'2px solid rgba(232,96,10,0.18)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 18px', overflow:'hidden', transition:'all 0.3s' }}>
                <Image src={cert.src} alt={cert.name} width={64} height={64} style={{ width:'78%', height:'78%', objectFit:'contain' }} />
              </div>
              <div style={{ fontFamily:'var(--font-oswald),sans-serif', fontSize:'clamp(16px,4vw,20px)', fontWeight:700, color:'#fff', marginBottom:9, letterSpacing:'1.2px' }}>{cert.name}</div>
              <p style={{ fontFamily:'var(--font-inter),sans-serif', fontSize:'clamp(11px,2.5vw,12px)', color:'rgba(255,255,255,0.38)', lineHeight:1.65, margin:'0 0 12px' }}>{cert.desc}</p>
